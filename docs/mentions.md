...
At query time, `MentionsQuery` loads the target entity via `get_entity` and extracts its matchable names (`registry.name`, `matchable=True` – typically `name`, `alias`, `previousName` …). The names go through the shared `clean_matching_names` cleaner in its default mode (`discard_single_token=True`) – the same mode the percolator transform uses – so short single tokens (e.g. `"Doe"`) and singles shadowed by a multi-token variant (e.g. `"Vladimir"` when `"Vladimir Putin"` is also present) are dropped before any phrase clause is built. The threshold for "short" is `OPENALEPH_SEARCH_MATCHING_SINGLE_TOKEN_MIN_LENGTH` (default `10`). Entity-vs-entity matching (`match_query` / `blocking_query`) uses the *recall* mode of the same cleaner — see [Matching](./matching.md#name-selection) — because it scores against normalized name fields rather than arbitrary prose. The cleaned set drives a `mention_clause` ANDed into the bool body of the same shape of search query a user would write against the documents bucket.
